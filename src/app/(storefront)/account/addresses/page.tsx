    ...
    <div className="space-y-6">
      ...
      {loading && (
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {[1, 2].map((n) => (
            <div
              key={n}
              className="h-44 rounded-3xl border border-slate-800 bg-slate-900/40 p-5 animate-pulse space-y-3"
            >
              <div className="h-4 w-1/3 rounded bg-slate-800" />
              <div className="h-3 w-3/4 rounded bg-slate-800/80" />
              <div className="h-3 w-1/2 rounded bg-slate-800/80" />
              <div className="h-8 w-full rounded-xl bg-slate-800/60 mt-4" />
            </div>
          ))}
        </div>
      )}
      ...
      {!loading && !fetchError && addresses.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-3xl border border-slate-800 bg-slate-900/60 p-12 text-center backdrop-blur-sm">
          ...
          <h3 className="text-base font-bold text-white mb-1">No Saved Addresses Yet</h3>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {addresses.map((addr) => (
            <div
              key={addr.id}
              className={[
                'relative flex flex-col justify-between rounded-3xl border p-5 transition-all duration-200 backdrop-blur-sm',
                addr.isDefault
              ...
              <div>
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-sm font-bold text-white">{addr.recipientName}</span>
                    {addr.isDefault && (
                      <span className="inline-flex items-center gap-1 text-[10px] font-extrabold uppercase tracking-wider text-blue-400 bg-blue-500/15 border border-blue-500/30 rounded-full px-2.5 py-0.5">
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  {addr.city}, {addr.state} &mdash; <span className="font-mono text-slate-300">{addr.postalCode}</span>
                </p>
                <p className="text-xs text-slate-400 mt-0.5">{addr.country}</p>
                <p className="text-xs font-mono text-slate-400 mt-2">Mobile: {addr.phone}</p>
              </div>

              {/* Action Buttons */}
              <div className="flex items-center justify-between border-t border-slate-800/80 pt-4 mt-4 gap-2">
                {!addr.isDefault ? (
                  <button
                    type="button"
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-xs font-medium text-slate-400 hover:text-white p-1.5 rounded-xl hover:bg-slate-800 transition-colors"
                    title="Edit Address"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 text-xs font-medium text-slate-400 hover:text-red-400 p-1.5 rounded-xl hover:bg-red-500/10 transition-colors"
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-lg rounded-3xl border border-slate-700 bg-slate-900 p-6 shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSaveAddress} className="space-y-4">
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                />
                <label htmlFor="modal-default-check" className="text-xs text-slate-300 font-medium">
              ...
              <div className="flex gap-3 pt-3 border-t border-slate-800">
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-60 disabled:cursor-not-allowed px-4 py-2.5 text-xs font-semibold text-white transition-colors shadow-lg shadow-blue-600/25"
                >
                  {submitting ? (
                ...
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="rounded-xl border border-slate-700 bg-slate-800 hover:bg-slate-700 px-4 py-2.5 text-xs font-semibold text-slate-300 transition-colors"
                >
                  Cancel
                </button>
              </div>
  ...
  return (
    <div className="space-y-1">
      ...
      <input
        ...
        maxLength={maxLength}
        className={[
          'w-full rounded-xl border bg-slate-900 px-3 py-2 text-xs text-white placeholder-slate-600 outline-none transition-colors',
          error
